...
editor: visual
params: 
  school: 110644 #	110644     139959
---

This demo uses IPEDS data pulled from the Urban Institute API (https://educationdata.urban.org/documentation/colleges.html#overview). We will pull IPEDS directory and financial aid data sets and create plots based on a few fields.
...
Density plots are difficult to make in B.I. tools, but `{ggridges}` and `geom_density()` make it easy to do in R.
...
Parameters let you set a variable when you render making it easy to cycle through many categories at once

```{r, echo=FALSE, message=FALSE, warning=FALSE}
fin_aid2$color_cat = if_else(fin_aid2$unitid == params$school, 'param','other')

ggplot(fin_aid2, aes(x=net_price, y=average_grant, color=color_cat)) + 
  geom_point() +
  scale_color_manual(values=c(
    "grey", "blue")) + 
    labs(
